...
elseif( strcmp( user.optfctn.solver, 'quadprog' ) == 1 )
  ...
  s_0 = state_encode( user, 0, u_p, d_p );
  dummyD = ones( Nmodes, Nsamples );
  s_L = state_encode( user, -Inf, repmat( user.u_min, 1, Nsamples ) - user.data.u, - user.data.d );
  s_U = state_encode( user, Inf, repmat( user.u_max, 1, Nsamples ) - user.data.u, dummyD - user.data.d );
  [ F, c, A, b_L, b_U ] = encode_LP_cons( user ); % generates the cost and matrics QP formulation
  ...
  if( res.inform ~= 1 )
    warning( 'Optimality condition not satisfied, this is not normal' );
  end
